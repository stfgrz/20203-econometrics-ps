...
```{r}
latvia <- readr::read_csv("../data/latvia_data.csv", col_types = cols())
```
...
```{r}
#| label: helper-faceted-time-series-plot
...
stationarity_results <- map_dfr(vars_to_test, function(v) {  clean_v <- na.omit(latvia[[v]])
  ts_v <- ts(clean_v, start = min(latvia$year), frequency = 1)
...
```{r}
# convert to ts
infl_clean <- na.omit(latvia$infl)
infl_ts <- ts(infl_clean, start=2001, frequency=1)
...
clean_lat <- na.omit(dplyr::select(latvia, infl, rGDP_pc, M2, unemp, cbrate))
lat_ts <- ts(clean_lat, start=2001, frequency=1)
...
var_subset <- na.omit(latvia[, c("infl", "M2")])
ec_term   <- ts(as.matrix(var_subset) %*% beta_hat, start=2001)
...
```{r}
# select variables
clean_var_subset <- na.omit(latvia[, c("infl", "rGDP_pc", "M2")])
var_data <- ts(
  clean_var_subset, 
  start     = 2001, 
  frequency = 1
...
```{r}
#| label: Prepare-inflation-series
...
```{r}
#| label: Rolling-one-step-AR2-forecasts
...
```{r}
#| label: Build-tibble-AR2-results
results_ar2 <- tibble(
...
```{r}
#| label: plot-actual-vs-both-forecasts
#| fig-width: 10
#| fig-height: 4
#| fig-dpi: 300
#| fig-align: center
